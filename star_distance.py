...
for fits_file in stars_files:
    ...
    distance_list, angle_list, std_list, std_angle = sf.file_distances(images, mask=True, outlier=out_list[file_counter-1],
    ...
    #                 xlabel='Time (s)', save=True, folder_path='./stars/dist_img2/')
    # sf.plt_file_ang(angle_list, name=name, exp_time=time, error=std_angle, ext='ang', ylabel='Angle (deg)',
    #                 xlabel='Time (s)', save=True, folder_path='./stars/dist_img2/')
    ...
    for tele in range(len(distance_list)):
        sf.plt_tele_stat(distance_list[tele], time, error=std_list[tele], name=name, telescope=tele+1,
                         ylabel='Distance', ext='d2s',
                         save=True, folder_path='./stars/dist2/')

        sf.plt_tele_hist(distance_list[tele], name=name, telescope=tele + 1,
                         xlabel='Distance', ext='h2s', save=True, folder_path='./stars/dist2/')

        sf.plt_tele_stat(angle_list[tele], time, error=std_angle[tele], name=name, telescope=tele + 1,
                         ylabel='Angle (deg)', ext='a2s',
                         save=True, folder_path='./stars/dist2/')
